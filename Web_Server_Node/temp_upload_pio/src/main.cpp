...
// Global objects for Keep-Alive stream connection
...
void sendFrameToServer() {
  ...
  if (!streamConnected) {
    streamClient.setInsecure(); // Bypass SSL cho ngrok HTTPS
    streamHttp.begin(streamClient, url);
    streamHttp.addHeader("Connection", "keep-alive");
    streamHttp.addHeader("ngrok-skip-browser-warning",
                         "true"); // QUAN TRONG: Bo qua trang cho cua ngrok
    streamHttp.addHeader("User-Agent", "ESP32-Camera");
    streamConnected = true;
  }

  streamHttp.addHeader("Content-Type", "application/octet-stream");
  streamHttp.addHeader("X-Device-ID", DEVICE_ID);
  streamHttp.addHeader("X-Frame-Width", "160");
  streamHttp.addHeader("X-Frame-Height", "120");
  streamHttp.addHeader("X-Frame-Format", "RGB565");
  streamHttp.setTimeout(2000);

  // Doc buffer B (an toan vi Core 1 dang ghi vao A)
  ...
  if (sendBuf) {
    int code = streamHttp.POST(sendBuf, 160 * 120 * 2);
    if (code == 200 || code == 201) {
      frameCount++;
    } else {
      Serial.printf("[STREAM] HTTP err: %d\n", code);
      streamHttp.end(); // Require reconnect on next frame
      streamConnected = false;
    }
  }
}
...
void captureAndUpload(String cmdId) {
  Serial.println("[CAPTURE] Uploading...");

  // Ngrok free tier va ESP32 co the tu choi 2 ket noi SSL song song
  // => Dong tam stream connection de nhuong duong cho Capture
  if (streamConnected) {
    streamHttp.end();
    streamConnected = false;
  }
  ...
  uint8_t *sendBuf = NULL;
  if (xSemaphoreTake(frameMutex, 100 / portTICK_PERIOD_MS) == pdTRUE) {
    sendBuf = frameBufferB;
    xSemaphoreGive(frameMutex);
  }

  if (!sendBuf) {
    sendACK(cmdId, "ERROR", "Buffer khong san sang");
    return;
  }
  ...
  client.setInsecure(); // Bypass SSL cho ngrok HTTPS
  http.begin(client, url);

  http.addHeader("Content-Type", "application/octet-stream");
  http.addHeader("ngrok-skip-browser-warning",
                 "true"); // QUAN TRONG: Bo qua trang cho cua ngrok
  http.addHeader("User-Agent", "ESP32-Camera-Capture");
  http.addHeader("X-Device-ID", DEVICE_ID);
  http.addHeader("X-Frame-Width", String(camW));
  http.addHeader("X-Frame-Height", String(camH));
  http.addHeader("X-Frame-Format", "RGB565");
  http.setTimeout(5000);

  int code = http.POST(sendBuf, camW * camH * 2);
  if (code == 200 || code == 201) {
    Serial.println("[CAPTURE] Upload OK!");
    sendACK(cmdId, "OK", "Da chup va upload thanh cong");
    frameCount++;
  } else {
    Serial.printf("[CAPTURE] Upload FAIL: %d\n", code);
