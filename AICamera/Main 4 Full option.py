...
# ------------------ CẤU HÌNH ĐƯỜNG DẪN ------------------
SAVE_DIR = r"D:\03. Thac si\04. AI\Luu anh chup"
LOG_IMG_DIR = r"D:\03. Thac si\04. AI\Image log"
LOGFILE_PATH = r"D:\03. Thac si\04. AI\Logfile.xlsx"
os.makedirs(SAVE_DIR, exist_ok=True)

# ------------------ HÀM TIỆN ÍCH ------------------
def imread_any(path: str) -> np.ndarray:
    """Đọc ảnh hỗ trợ tên tiếng Việt"""
...
def capture_image():
    ...
    choice = messagebox.askquestion("Chọn chế độ", "Bạn muốn nhận diện (Yes) hay lưu ảnh (No)?")
    if choice == "yes":
        lbl_status.config(text="🧠 Đang nhận diện sản phẩm...")
        root.update()
        img_bgr = cv2.cvtColor(frame_data, cv2.COLOR_RGB2BGR)
        name = identify_object_from_log(img_bgr)
        lbl_status.config(text=f"📦 Sản phẩm: {name}")
        messagebox.showinfo("Kết quả nhận diện", f"Sản phẩm được nhận diện là: {name}")
    else:
...
def extract_background():
    ...
    if test_image is None:
        messagebox.showwarning("Chưa chọn ảnh", "Hãy bật 'Mode tách nền' và chọn ảnh trước.")
        return
    bgr = cv2.cvtColor(test_image, cv2.COLOR_RGB2BGR)
    _, result = segment_object(bgr)
    result_rgb = cv2.cvtColor(result, cv2.COLOR_BGR2RGB)
    display_image(result_rgb)
    lbl_status.config(text="✅ Đã tách nền thành công")
...
Button(frm_buttons, text="✂️ Tách nền", command=extract_background,
